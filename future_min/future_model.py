...
def inference(features, hidden1_units, hidden2_units):
    ...
    Args:
      features: features placeholder, from inputs()       hidden1_units: Size of the first hidden layer.
...
def evaluation(logits, labels):
    ...
    Args:
      logits: Logits    nsor, float - [batch_size, NUM_CLASSES].
...
def read_single_data(filename):
    with open(filename, 'r') as csvfile:
        ...
        for item in data_list:
            item_feature = list(map(float, item[:7]))
            item_feature = list(map(int, item_feature))
            item_label = int(item[7])
