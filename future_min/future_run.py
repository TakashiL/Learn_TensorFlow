...
from future_min.future_model import *
...
def placeholder_inputs(batch_size):
    ...
    images_placeholder = tf.placeholder(tf.float32, shape=(batch_size, NUM_FEATURES))
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        lossfn = loss(logits, labels_placeholder)

        # Add to the Graph the Ops that calculate and apply gradients.
        train_op = training(lossfn, FLAGS.learning_rate)
        ...
        for step in xrange(FLAGS.max_steps):
            ...
            _, loss_value = sess.run([train_op, lossfn], feed_dict=feed_dict)
